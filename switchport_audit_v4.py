...
def access_parse(output):
    ...
    #determine access list w/o root guard # 
        is_acc_int = list(set(access_list)-set(storm_list))
    #determine trunk list w/o root guard # 
    ...
    #Update Interfaces removing ints in PO and with descr of gsc 
        ...
        f.close()
        apply_config(host)
        after_chck_ints(host)
        diff_check(file1,file2)
...
def b4_chck_ints(host):
    snmpwalk = ('./snmpwalk.sh')
    file = (">> " + "int_chk/" + host + "before_int_check.txt")
    intf_check = os.system(snmpwalk + " " + host + file )
    file1 = "int_chk/"+host+"before_int_check.txt"
    return file1

def after_chck_ints(host):
    snmpwalk = ('./snmpwalk.sh')
    file = (">> " + "int_chk/" + host + "after_int_check.txt")
    intf_check = os.system(snmpwalk + " " + host + file )
    file2 = "int_chk/"+host+"before_int_check.txt"
    return file2

def diff_check(file1,file2):
    diff = ('diff')
    diff_check = os.system(diff + " "+ file1 +" "+ file2)
    print diff_check
    return diff_check
